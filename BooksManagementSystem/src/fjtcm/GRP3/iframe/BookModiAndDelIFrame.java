...
public class BookModiAndDelIFrame extends JInternalFrame {
	...
	public BookModiAndDelIFrame() {
		...
		setClosable(true);
		setTitle("图书信息修改与删除");
		setBounds(100, 100, 1030, 450);// 移动组件并调整其大小。由 x 和 y 指定左上角的新位置，由 width 和
										// height 指定新的大小。
		...
		headLogo.setPreferredSize(new Dimension(1030, 80));
	...
	class addBookActionListener implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			...
			int i = Dao.Updatebook(ISBNs, bookTypes, bookNames, writers, translators, publishers,
					Date.valueOf(pubDates), Double.parseDouble(prices));
