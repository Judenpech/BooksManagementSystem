...
import fjtcm.GRP3.model.BookInfo;
import fjtcm.GRP3.model.BookType;
import fjtcm.GRP3.model.Operator;
import fjtcm.GRP3.model.Reader;
import fjtcm.GRP3.model.User;
...
public class Dao {
	...
	public static Operator check(String id, String password) {
		...
		String sql = "select *  from tb_operator where id='" + id + "' and password='" + password + "'and admin=1";
	...
	public static int Insertoperator(String name, String sex, int age, String identityCard, Date workdate, String tel,
		...
		int i = 0;
		try {
			String sql = "insert into tb_operator(name,sex,age,identityCard,workdate,tel,password,admin) values('"
	...
	public static List selectuser() {
		List list = new ArrayList();
		String sql = "select id,name,sex,age,identityCard,workdate,tel,password  from tb_operator where admin=1";
		ResultSet rs = Dao.executeQuery(sql);
		try {
			while (rs.next()) {
				User user = new User();
	...
	// 更新管理员信息
	public static int Updateuser(int id, String name, String sex, int age, Date workdate, String tel, String password) {
		int i = 0;
		try {
			String sql = "update tb_operator set name='" + name + "',sex='" + sex + "',age=" + age + ",workdate='"
					+ workdate + "',tel='" + tel + "',password='" + password + "' where id='" + id + "'";
			i = Dao.executeUpdate(sql);
		} catch (Exception e) {
			e.printStackTrace();
		}
